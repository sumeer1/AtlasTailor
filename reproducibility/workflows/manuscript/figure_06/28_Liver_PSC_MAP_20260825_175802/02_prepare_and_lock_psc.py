#!/usr/bin/env python3
"""Apply the exact post-MASLD frozen model to PSC anchors and geometry, then lock."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/mplconfig_liver_psc_map")
...
def main():
    ...
    programs_path = ROOT / "PSC_PROGRAMS.json"
    ...
    for sample, (gsm, stem) in SAMPLES.items():
        ...
        for seed, metadata in registrations.items():
            registration_rows.append({
                "patient": "PSC011", "sample": sample, "seed": int(seed),
    ...
    print(json.dumps({"status": "ALL_PSC_PREDICTIONS_LOCKED", "sections": len(manifest_rows),
                      "patient_n": 1, "anchors": 16, "hidden_genes": len(evaluation),
