#!/usr/bin/env python3
"""Verify all AH locks, then open hidden expression and evaluate frozen endpoints."""
...
def verify_all_locks():
    ...
    if len(table) != 2:
        raise RuntimeError(f"Expected two AH locks, found {len(table)}")
    checks, cache = [], {}
    ...
    if not all(item["verified"] for item in checks):
        raise RuntimeError("AH prediction-lock verification failed")
...
def main():
    ...
    program_spec = json.loads((ROOT / "AH_PROGRAMS.json").read_text())["programs"]
    ...
    for sample, (gsm, stem) in SAMPLES.items():
        ...
        section_rows.append({
            "disease": "AH", "patient": sample, "sample": sample, "GSM": gsm,
        ...
        for program, genes in program_spec.items():
            ...
            program_rows.append({
                "patient": sample, "sample": sample, "program": program,
    ...
    sections.to_csv(ROOT / "04_PATIENT_LEVEL_RESULTS.tsv", sep="\t", index=False)
    ...
    qc.to_csv(ROOT / "00B_AH_QC.tsv", sep="\t", index=False)
    ...
    patient_spatial = bool((sections["median_spatial_map"] > sections["median_spatial_idw"]).all())
    patient_rmse = bool((sections["log1p_rmse_map"] < sections["log1p_rmse_idw"]).all())
    patient_residual = bool((sections["pooled_residual_pearson_map"] > 0).all())
    ...
    if patient_spatial and patient_rmse and patient_residual and all_programs_positive:
        ...
        decision = "C"
    summary = {
        "AH_decision": decision, "independent_patient_n": 2, "sections": 2,
        ...
        "compartment_analysis": "UNAVAILABLE_NO_DEPOSITED_SPOT_ANNOTATIONS",
        "prediction_lock": "PASS", "leakage_audit": "PASS",
    }
    (ROOT / "ah_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
