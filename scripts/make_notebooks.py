#!/usr/bin/env python3
"""Generate compact internal notebooks for CI fixture validation."""
from pathlib import Path
import nbformat as nbf

ROOT = Path(__file__).resolve().parents[1]
NB = ROOT / "tests" / "notebooks"
...
notebook("00_quickstart_synthetic.ipynb", "CI fixture: quickstart", "Adapt a tiny target while target non-anchor columns remain inaccessible to prediction.", [
    setup2,
...
notebook("01_design_sparse_panel_synthetic.ipynb", "CI fixture: sparse panel design", "Source-only nested spatial cross-validation evaluates specified measurement budgets; it does not establish a universal optimum.", [
    setup2,
...
notebook("02_adapt_3d_specimen_synthetic.ipynb", "CI fixture: 3D adaptation", "The target contributes geometry and measured anchors; anchors enter only after geometry registration.", [
    setup3,
    "panel = hs.design_panel(reference, budget=8, config=hs.DesignConfig(budget=8, budgets=(8,)))\ntarget = read_target(EX / 'target.h5ad', panel.genes)",
...
notebook("03_explore_target_specific_residuals_synthetic.ipynb", "CI fixture: target-specific residuals", "Residual structure is variation beyond the registered atlas expectation and is not automatically biological.", [
    setup2,
    "panel = hs.design_panel(reference, budget=8, config=hs.DesignConfig(budget=8, budgets=(8,)))\ntarget = read_target(EX / 'target.h5ad', panel.genes)\nresult = hs.adapt(reference, target, panel.genes, config=hs.AdaptConfig(registration='pre_registered', seeds=(42,)))",
...
notebook("04_forecast_with_MAPT_synthetic.ipynb", "CI fixture: temporal forecasting", "A compatible tracking map transports the individualized early field; the frozen source-trained temporal residual supplies the supported correction.", [
    setup2,
...
notebook("05_adapt_2d_visium_synthetic.ipynb", "CI fixture: 2D adaptation", "The 2D adapter changes coordinate dimensionality and spatial folds, not the core atlas-prior/residual/guard logic.", [
    setup2,
    "panel = hs.design_panel(reference, budget=8, config=hs.DesignConfig(budget=8, budgets=(8,)))\ntarget = read_target(EX / 'target.h5ad', panel.genes)",
